...
# ================= NOTEBOOK CELL 4 =================
# ============================================================
# FULL FINGERPRINT TRAINING SCRIPT
# - Uses ALL fingerprint images
# - Zero-pads subject IDs (e.g. 5 -> 005)
# - Trains on train split
# - Uses common val pairs for model selection
# - Saves best model for embedding extraction
# ============================================================
...
PAIR_DIR = "/kaggle/working/common_pairs"
SAVE_DIR = "/kaggle/working/fingerprint_training"

TRAIN_CSV = os.path.join(SPLIT_DIR, "train_subjects.csv")
VAL_CSV = os.path.join(SPLIT_DIR, "val_subjects.csv")
VAL_PAIR_CSV = os.path.join(PAIR_DIR, "val_pairs_common.csv")

os.makedirs(SAVE_DIR, exist_ok=True)

BEST_MODEL_PATH = os.path.join(SAVE_DIR, "best_fingerprint_model.pth")
LAST_MODEL_PATH = os.path.join(SAVE_DIR, "last_fingerprint_model.pth")
HISTORY_PATH = os.path.join(SAVE_DIR, "training_history.csv")
CLASS_MAP_PATH = os.path.join(SAVE_DIR, "train_class_mapping.csv")
...
def read_subject_ids(csv_path):
    ...
    ids = df.iloc[:, 0].dropna().apply(clean_sid).tolist()
    return ids
...
def list_fingerprint_images(subject_id, dataset_root):
    sid = clean_sid(subject_id)
    folder = os.path.join(dataset_root, sid, "fingerprint")
...
def build_train_dataframe(subject_ids, dataset_root):
    ...
    for sid in tqdm(subject_ids, desc="Scanning train fingerprint images"):
        imgs = list_fingerprint_images(sid, dataset_root)
...
print("Skipped train subjects with no fingerprint images:", len(skipped_train))
print(train_df.head())

if len(train_df) == 0:
    raise ValueError("No training images found. Check DATASET_ROOT or folder structure.")
...
# 6. DATASETS
# =========================
class FingerprintTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = load_gray_to_rgb(row["image_path"])
        if self.transform:
            img = self.transform(img)
        label = int(row["label"])
        return img, label

train_dataset = FingerprintTrainDataset(train_df, transform=train_transform)
...
model = FingerprintEmbeddingNet(num_classes=num_classes, embed_dim=EMBED_DIM).to(device)
...
# =========================
@torch.no_grad()
def extract_subject_embeddings(model, subject_ids, dataset_root, transform, device):
    model.eval()
    emb_map = {}

    for sid in tqdm(subject_ids, desc="Extracting val embeddings", leave=False):
        img_paths = list_fingerprint_images(sid, dataset_root)

        for idx, path in enumerate(img_paths):
            img = load_gray_to_rgb(path)
...
def evaluate_on_common_pairs(model, pair_df, subject_ids, dataset_root, transform, device):
    emb_map = extract_subject_embeddings(model, subject_ids, dataset_root, transform, device)
    ...
    for _, row in pair_df.iterrows():
        ...
        if key1 not in emb_map or key2 not in emb_map:
            continue
    ...
    if len(scores) == 0:
        raise ValueError("No validation pairs could be evaluated. Check pair CSV and image indexing.")
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_metrics["eer"] < best_eer:
        ...
        val_score_df.to_csv(os.path.join(SAVE_DIR, "best_val_scores.csv"), index=False)
...
# ================= NOTEBOOK CELL 5 =================
# ============================================================
# FINGERPRINT: EXTRACT EMBEDDINGS FOR TRAIN/VAL/TEST
# + CALCULATE TEST SCORES
# + EVALUATE METRICS AND CURVES
# ============================================================
...
SPLIT_DIR = "/kaggle/working/splits"
PAIR_DIR = "/kaggle/working/common_pairs"
MODEL_DIR = "/kaggle/working/fingerprint_training"
SAVE_DIR = "/kaggle/working/fingerprint_final_outputs"

TRAIN_CSV = os.path.join(SPLIT_DIR, "train_subjects.csv")
VAL_CSV = os.path.join(SPLIT_DIR, "val_subjects.csv")
TEST_CSV = os.path.join(SPLIT_DIR, "test_subjects.csv")

TEST_PAIR_CSV = os.path.join(PAIR_DIR, "test_pairs_common.csv")

BEST_MODEL_PATH = os.path.join(MODEL_DIR, "best_fingerprint_model.pth")
...
def build_image_dataframe(subject_ids, dataset_root, split_name="split"):
    ...
    for sid in tqdm(subject_ids, desc=f"Scanning {split_name} fingerprint images"):
        imgs = list_fingerprint_images(sid, dataset_root)
...
if len(all_df) == 0:
    raise ValueError("No fingerprint images found.")
...
def make_loader(df):
    ds = FingerprintImageDataset(df, transform=eval_transform)
    loader = DataLoader(
        ds,
        batch_size=BATCH_SIZE,
        shuffle=False,
...
print("Best val EER:", ckpt.get("best_eer", "N/A"))
...
def extract_embeddings(model, loader, split_name="split"):
    ...
    pbar = tqdm(loader, total=len(loader), desc=f"Extracting {split_name} embeddings")
    for imgs, subjects, img_indices, paths in pbar:
        imgs = imgs.to(device, non_blocking=True)

        embs = model(imgs, return_embedding=True)
...
np.save(os.path.join(SAVE_DIR, "fingerprint_train_embeddings.npy"), train_embs)
np.save(os.path.join(SAVE_DIR, "fingerprint_val_embeddings.npy"), val_embs)
np.save(os.path.join(SAVE_DIR, "fingerprint_test_embeddings.npy"), test_embs)
np.save(os.path.join(SAVE_DIR, "fingerprint_all_embeddings.npy"), all_embs)

train_meta.to_csv(os.path.join(SAVE_DIR, "fingerprint_train_embeddings_meta.csv"), index=False)
val_meta.to_csv(os.path.join(SAVE_DIR, "fingerprint_val_embeddings_meta.csv"), index=False)
test_meta.to_csv(os.path.join(SAVE_DIR, "fingerprint_test_embeddings_meta.csv"), index=False)
all_meta.to_csv(os.path.join(SAVE_DIR, "fingerprint_all_embeddings_meta.csv"), index=False)

# =========================
# 9. BUILD TEST EMBEDDING MAP
# =========================
test_emb_map = {}
for i, row in test_meta.iterrows():
    key = (clean_sid(row["subject"]), int(row["img_idx"]))
    test_emb_map[key] = test_embs[i]

print("\nTest embedding map size:", len(test_emb_map))

# =========================
# 10. COMPUTE TEST SCORES
# =========================
scores = []
labels = []
pair_ids = []
...
for _, row in tqdm(test_pairs_df.iterrows(), total=len(test_pairs_df), desc="Computing test scores"):
    pair_id = int(row["pair_id"])
    s1 = clean_sid(row["subject1"])
    s2 = clean_sid(row["subject2"])
    idx1 = int(row["idx1"])
...
score_csv_path = os.path.join(SAVE_DIR, "fingerprint_test_scores.csv")
...
if len(scores) == 0:
    raise ValueError("No valid test pairs could be scored.")
...
metrics = {
    ...
    "num_all_images": int(len(all_df))
}

with open(os.path.join(SAVE_DIR, "fingerprint_test_metrics.json"), "w") as f:
...
plt.title("Fingerprint Test ROC Curve")
plt.legend(loc="lower right")
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "fingerprint_test_roc_curve.png"), dpi=200)
plt.close()

# =========================
# 13. SCORE DISTRIBUTION CURVE
# =========================
...
plt.axvline(threshold, linestyle="--", label=f"Thr={threshold:.4f}")
plt.xlabel("Cosine Similarity Score")
plt.ylabel("Density")
plt.title("Fingerprint Test Score Distribution")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "fingerprint_test_score_distribution.png"), dpi=200)
plt.close()

# =========================
# 14. OPTIONAL: SAVE ROC POINTS
# =========================
roc_df = pd.DataFrame({
    "fpr": fpr,
    "tpr": tpr,
...
roc_df.to_csv(os.path.join(SAVE_DIR, "fingerprint_test_roc_points.csv"), index=False)

# =========================
# 15. SAVE SUMMARY TXT
# =========================
summary_path = os.path.join(SAVE_DIR, "fingerprint_test_summary.txt")
with open(summary_path, "w") as f:
    f.write("FINGERPRINT TEST SUMMARY\n")
    f.write("========================\n")
...
# ================= NOTEBOOK CELL 29 =================
# ============================================================
# FINGERPRINT TRAINING SCRIPT (BALANCED VAL PAIRS)
# - Uses all fingerprint images
# - Uses balanced validation pairs
# - Prints Train Loss, Train Acc, Val Loss, Val EER, Val Acc
# - Saves best model by lowest validation EER
# ============================================================
...
SAVE_DIR = "/kaggle/working/fingerprint_training_balanced"
...
def evaluate_on_common_pairs(model, pair_df, subject_ids, dataset_root, transform, device):
    ...
    for _, row in pair_df.iterrows():
        ...
        # map cosine score [-1,1] to logit-like value for BCE monitoring
    ...
    if len(scores) == 0:
        raise ValueError("No validation pairs could be evaluated.")

    eer, threshold = compute_eer(scores, labels)
    preds = (scores >= threshold).astype(int)
    acc = accuracy_score(labels, preds)

    # val loss for monitoring
    val_loss = val_bce(
...
# ================= NOTEBOOK CELL 30 =================
# ============================================================
# FINGERPRINT BALANCED TEST EVALUATION
# - Load best trained model
# - Extract embeddings for train / val / test / all images
# - Compute scores on BALANCED test pairs
# - Evaluate metrics
# - Show ROC, score distribution, confusion matrix on screen
# - Save all outputs
# ============================================================
...
SPLIT_DIR = "/kaggle/working/splits"
PAIR_DIR = "/kaggle/working/common_pairs_balanced"
MODEL_DIR = "/kaggle/working/fingerprint_training_balanced"
SAVE_DIR = "/kaggle/working/fingerprint_balanced_test_outputs"
...
print("Val IDs:", len(val_ids))
print("Test IDs:", len(test_ids))
print("Balanced test pairs:", test_pairs_df.shape)
...
# =========================
# 10. COMPUTE BALANCED TEST SCORES
# =========================
scores = []
labels = []
pair_ids = []
...
for _, row in tqdm(test_pairs_df.iterrows(), total=len(test_pairs_df), desc="Computing balanced test scores"):
    pair_id = int(row["pair_id"])
    s1 = clean_sid(row["subject1"])
    s2 = clean_sid(row["subject2"])
    idx1 = int(row["idx1"])
...
score_csv_path = os.path.join(SAVE_DIR, "fingerprint_test_scores_balanced.csv")
score_df.to_csv(score_csv_path, index=False)
...
with open(os.path.join(SAVE_DIR, "fingerprint_test_metrics_balanced.json"), "w") as f:
    json.dump(metrics, f, indent=4)
...
plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate")
plt.title("Fingerprint Balanced Test ROC Curve")
plt.legend(loc="lower right")
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "fingerprint_test_roc_curve_balanced.png"), dpi=200)
plt.close()
...
plt.xlabel("Cosine Similarity Score")
plt.ylabel("Density")
plt.title("Fingerprint Balanced Test Score Distribution")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "fingerprint_test_score_distribution_balanced.png"), dpi=200)
plt.close()
...
plt.title("Fingerprint Balanced Test Confusion Matrix")
plt.colorbar()
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "fingerprint_test_confusion_matrix_balanced.png"), dpi=200)
plt.close()

# =========================
# 15. SAVE ROC POINTS + SUMMARY
# =========================
roc_df = pd.DataFrame({
    "fpr": fpr,
    "tpr": tpr,
...
roc_df.to_csv(os.path.join(SAVE_DIR, "fingerprint_test_roc_points_balanced.csv"), index=False)

summary_path = os.path.join(SAVE_DIR, "fingerprint_test_summary_balanced.txt")
with open(summary_path, "w") as f:
    f.write("FINGERPRINT BALANCED TEST SUMMARY\n")
    f.write("================================\n")
    for k, v in metrics.items():
        f.write(f"{k}: {v}\n")
